...
func ConnPostgresORM(dbHost, dbUser, dbPass, dbName, dbTimeZone string, maxIdle, maxConn int) (*gorm.DB, error) {
	// val := url.Values{}
	// val.Add("TimeZone", "Asia/Jakarta")
	// dsn := fmt.Sprintf("%s&%s", desc, val.Encode())

	desc := getConnectionStringORM(dbHost, dbUser, dbPass, dbName, dbTimeZone)
...
func getConnectionStringORM(dbHost, dbUser, dbPass, dbName, dbTimeZone string) string {
